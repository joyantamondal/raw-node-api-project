 ...
 * Description: handler to handle users define checks
...
handler._check.post = (requestProperties, callback) => {
    ...
    const method = typeof(requestProperties.body.method) === 'string' && ['GET','POST','PUT','DELETE'].indexOf
    (requestProperties.body.method) > -1 ? requestProperties.body.method : false;
...
data.read('checks', id,(err,checkData)=>{
    if(!err && checkData){
         //verify token
         const token = typeof(requestProperties.headersObject.token) === 'string' ? requestProperties.headersObject.token : false;
         tokenHandler._token.verify(token,parseJSON(checkData).userPhone,(tokenValid)=>{
            if(tokenValid){
                    ...
                    if(!err2){
                        data.read('users',parseJSON(checkData).userPhone,(err3,userData)=>{
                            ...
                            if(!err3 && userData){
                                 ...
                                 //remove the deleted check id users list of checks
